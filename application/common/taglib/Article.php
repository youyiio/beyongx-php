...
class Article extends TagLib
{
    ...
    public function tagSearch($tag, $content)
    {
        ...
        $parse .= "    array_push(\$cids, $internalCid);";
        $parse .= "    \$field = 'cms_article.id,title,description,author,thumb_image_id,post_time,read_count,comment_count';";
        $parse .= "    $internalList = \$ArticleModel->hasWhere('CategoryArticle', [['category_id','in',\$cids]], \$field)->where(\$where)->whereLike('title','%'.$keyword.'%','and')->field(\$field)->order(\$order)->paginate(10,false,['query'=>input('param.')]);";
        $parse .= "  } else { ";
        $parse .= "    $internalList = \$ArticleModel->where(\$where)->whereLike('title','%'.$keyword.'%','and')->field(\$field)->order(\$order)->paginate(10,false,['query'=>input('param.')]);";
        $parse .= "  };";
